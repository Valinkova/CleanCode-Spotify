...
public class SpotifyServer {
    private static final Logger log = LoggerFactory.getLogger(SpotifyServer.class);
        ...
        try (ServerSocket serverSocket = new ServerSocket(PORT)) {
            ...
            while (true) {
                try (Socket clientSocket = serverSocket.accept()) {
                    log.info("Accepted socket from {}", clientSocket.getInetAddress().getHostName());
    ...
    public static void main(String[] args) {
        ApplicationContext applicationContext = SpringApplication.run(SpotifyServer.class, args);
        applicationContext.getBean(SpotifyServer.class).run();
    }
}
